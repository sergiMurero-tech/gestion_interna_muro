...
function formatFecha(x: string, lang: 'va' | 'es') {
  return new Date(x).toLocaleDateString(lang === 'va' ? 'ca' : 'es-ES', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
  })
...
export default function PublicHome() {
  ...
  const ACCESOS = [
    {
      label: t('accesos.area.label'),
      desc: t('accesos.area.desc'),
      to: '/area-deportiva',
      icon: (
    ...
    {
      label: t('accesos.noticias.label'),
      desc: t('accesos.noticias.desc'),
      to: '/noticias',
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    ...
    {
      label: t('accesos.inscripciones.label'),
      desc: t('accesos.inscripciones.desc'),
      to: '/inscripciones',
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    ...
    {
      label: t('accesos.contacto.label'),
      desc: t('accesos.contacto.desc'),
      to: '/club/contacto',
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    ...
    <>
        ...
        <div className="relative mx-auto grid min-h-[60vh] max-w-6xl items-center gap-8 px-4 py-20 sm:min-h-[70vh] lg:grid-cols-[1.2fr_0.8fr]">
          <div className="animate-fade-up">
            ...
            <h1 className="h-display text-shadow text-6xl text-white sm:text-7xl lg:text-8xl">
              {t('home.lema_top')} <br />
              <span className="text-gold">{t('home.lema_bottom')}</span>
            </h1>
            ...
            >
              <div className="relative aspect-[4/5] sm:aspect-[3/4]">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-5 text-white">
                  <span className="text-xs uppercase tracking-widest text-gold">{formatFecha(hero.fecha_publicacion, lang)}</span>
                  <h2 className="mt-1 text-2xl font-extrabold leading-tight text-shadow line-clamp-3">{pickLang(hero, 'titulo', lang)}</h2>
                </div>
              </div>
            </Link>
          )}
      ...
      <section className="mx-auto max-w-6xl px-4 py-16">
        <div className="mb-8 flex items-end justify-between">
          <div>
            <span className="eyebrow">{t('home.accesos.eyebrow')}</span>
            <h2 className="section-title">
              {t('home.accesos.title.before')} <span className="text-gold">{t('home.accesos.title.after')}</span>
            </h2>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-4 pb-16">
        <div className="mb-8 flex items-end justify-between">
          <div>
            <span className="eyebrow">{t('home.ultimas.eyebrow')}</span>
            <h2 className="section-title">
              {t('home.ultimas.title.before')} <span className="text-gold">{t('home.ultimas.title.after')}</span>
            </h2>
          </div>
