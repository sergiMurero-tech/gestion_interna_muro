...
export default function DirectivaPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <h1 className="mb-6 text-3xl font-extrabold text-white">Directiva</h1>
      {directivos.length === 0 ? (
        <div className="card p-6 text-center text-zinc-400">No hay miembros de la directiva.</div>
      ) : (
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {directivos.map((d) => (
            <div key={d.id} className="card overflow-hidden p-0">
              ...
              <div className="p-4">
                <h2 className="font-bold text-white">{d.nombre}</h2>
                <p className="text-sm text-gold">{d.cargo}</p>
                {d.descripcion && <p className="mt-2 text-sm text-zinc-400">{d.descripcion}</p>}
              </div>
            </div>
          ))}
        </div>
